...
import { SETTING_ALERT_SOUND, TEST_ALERT_SOUND, TEST_STUDY } from './settings/types';
import { prolificStudiesUpdate } from './prolific/actions';
...
export const settingsAlertSoundMiddleware: Middleware = (store) => (next) => (action) => {
  ...
  if (action.type === TEST_STUDY) {
    const state: AppState = store.getState();
    updateResults([{average_completion_time: 5,
